...
(defvar prolog-mode-version "1.12"
...
;; (FSF Emacs and XEmacs <21.4), or ~/.xemacs/init.el (XEmacs
...
;;   have a smaller chance to notice this kind of bugs otherwise.

;; Changelog:

;; Version 1.11 / 1.12
;;  o  FSF Emacs compatibility fix for paragraph filling (fixed
...
;;     elegant (i.e., without compilation warnings) in the future.
;; Version 1.2:
...
(defcustom prolog-char-quote-workaround t
...
(defun prolog-guess-fill-prefix ()
  ;; fill 'txt entities?
  (when (equal (prolog-in-string-or-comment) 'cmt)
    (let (beg end str)
      (save-excursion
        (end-of-line)
        (setq end (point))
...
;; FSF Emacs compatibility: FSF Emacs does not differentiate between
...
during the life of an Emacs session, and because FSF Emacs wants them
so by ignoring `easy-menu-add'."

  ;; FSF Emacs ignores `easy-menu-add' so the order in which the menus
