    ...
    (let ((space (skip-chars-backward " \t"))
	  ...
	  (and (looking-at "\\?")
	       (or (eq (char-syntax (char-before (point))) ?w)
                   ;; i can't figure out what the hell ruby-special-char-p is testing.
		   (ruby-special-char-p))))
	nil)
       ...
       ((or (looking-at ruby-operator-re)
	    (looking-at "[\\[({,;]")
	    (and (looking-at "[!?]")
                 ;; modifier is like rescue, ensure
...
(defun ruby-forward-string (term &optional end no-error expand)
  (let ((n 1) ;; string balancing depth
        (c (string-to-char term))
	(re (if expand
		(concat "[^\\]\\(\\\\\\\\\\)*\\([" term "]\\|\\(#{\\)\\)")
                ;; this regexp is something like [^\](\\)*
                ;; it stops at \, or skip over all \\*
	      (concat "[^\\]\\(\\\\\\\\\\)*[" term "]"))))
    (while (and (re-search-forward re end no-error)
		(if (match-beginning 3) ; if we are in interpolation
		    (ruby-forward-string "}{" end no-error nil) ; huh... funny. I'll just assume this works.
...
(defun ruby-parse-partial (&optional end in-string nest depth pcol indent)
  ...
  (when (re-search-forward ruby-delimiter end 'move)
    (let ((pnt (point)) ; end of delimiter (exclusive)
          w
          re
          expand ; marks string with interpolation
          )
      ; start examining from the beginning of delimiter (inclusive)
	...
	(goto-char pnt))
       ((looking-at (concat "\\<\\(" ruby-block-end-re "\\)\\>"))
        ;; i don't understand why this test condition is necessary. Aren't we using \< and \> already?
	(if (or (and (not (bolp))
		     (progn
		       (forward-char -1)
	...
	(t
	 (add-hook 'ruby-mode-hook
	    '(lambda ()
	       ...
	       (setq font-lock-syntactic-keywords ruby-font-lock-syntactitac-keywords)))))
